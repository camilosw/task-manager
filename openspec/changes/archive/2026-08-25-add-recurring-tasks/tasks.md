...
- [x] 14.1 Run the full suite and confirm it is green, with no skipped or commented-out tests.
      Result: `npm test` — 22 files, 316 tests, all passing; `grep` for `.skip`/`.only`/`xit`/`xdescribe`/`test.todo` across the test tree found none. `npm run typecheck` (`tsc -b`) and `npm run lint` (`eslint .`) both clean. One run out of five showed a single `waitFor` timeout in `Rollover.test.tsx`'s pre-existing (non-recurring) "foreground rollover (10.1)" test; it passed in isolation and on four full-suite re-runs with no code change, so it reads as environment-timing flakiness under load rather than a regression — reported, not treated as a fix target.
- [x] 14.2 By hand: create "every Monday" and "first Monday of the month" tasks in the running application, confirm each appears in Today on its date, completes, disappears, and returns at the next occurrence. Verified by changing the device date, since the day boundary cannot be crossed in the test environment.
      Method: browser-based check — `npm run dev` (Vite) plus Playwright (`playwright-cli`, Chromium) against it, with the page's global `Date` overridden via an injected init script (`window.__setMockNow`) rather than touching the host's real clock. Result: on mocked Monday 3 Aug 2026, created "Weekly review" (weekly/Monday) and "Monthly kickoff" (monthly/first Monday) — both appeared immediately in Today under "Recurring" with the correct echoed rule ("Every Mon" / "1st Mon"); completing each struck them through in Today while both remained visible there and in All, and neither appeared in Completed. Advancing the mocked date to Monday 10 Aug and clicking "Recalculate today" returned "Weekly review" to Today as pending while "Monthly kickoff" correctly stayed at rest (10 Aug is not the first Monday of August). Advancing to Monday 7 Sep and firing a `visibilitychange` recompute returned both tasks to Today as pending, since 7 Sep 2026 is both a Monday and the first Monday of September. Matches the dates traced in specs/recurring-tasks/spec.md.
- [x] 14.3 By hand: confirm a database written by the previous version opens after the upgrade with every task intact and no Recurring group displayed.
      Method: browser-based check — seeded IndexedDB `task-manager` at version 2 directly (mirroring the `seedVersion2Database` helper in `src/persistence/indexedDbRepository.test.ts`) with three plain tasks (medium/pending, urgent/pending, high/completed) plus a stored snapshot, then loaded the app so it opened the database at version 3. Result: all three tasks rendered intact — names, durations, priorities, and completion states unchanged — under their ordinary priority groups in Today, All, and Completed; no "Recurring" heading appeared anywhere. A direct read of the upgraded store confirmed every task now carries `recurrence: null` and `lastCompletedOn: null` with every other field (including `place`) untouched.
- [x] 14.4 Confirm every requirement in the six spec deltas has at least one test or a recorded manual check, and note any that shipped differently from the specs.
      Audited all six spec deltas against the test suite and the 14.2/14.3 manual checks; no functional gap or spec deviation found. Two minor process notes (not implementation gaps): the 13.2 by-hand contrast check was never recorded — computed now from the shipped OKLCH values (light: marker vs page bg 4.99:1, badge fg vs badge bg 8.84:1; dark: 7.01:1 / 9.48:1 — comparable to or better than the existing priority tokens' contrast), confirming it passes but the record was missing until this section. A few structurally-guaranteed scenarios have no dedicated test of their own — "a rule edit leaves today's plan alone" (recurring-tasks spec) and "a new recurring task takes the last place among recurring tasks" (task-management spec) — because both fall out of pre-existing, already-tested machinery (`editTask` never touches `plannedIds`; `nextPlace` is a single global sequence) rather than new logic, and "no occurrence is ever announced" (recurring-tasks spec) has no notification code anywhere in the app to test the absence of.
